...
const taskColumns = `id, name, status, project, branch, prompt, backend, worktree, agent_pid, session_id, pr_url, sandboxed, archived, waiting_review, runtime, remote_host, created_at, started_at, ended_at`
...
func scanTask(row scanner) (*model.Task, error) {
	t := &model.Task{}
	var status, createdAt, startedAt, endedAt, runtime string
	var sandboxed, archived, waitingReview int
	if err := row.Scan(&t.ID, &t.Name, &status, &t.Project, &t.Branch, &t.Prompt, &t.Backend, &t.Worktree, &t.AgentPID, &t.SessionID, &t.PRURL, &sandboxed, &archived, &waitingReview, &runtime, &t.RemoteHost, &createdAt, &startedAt, &endedAt); err != nil {
		return nil, err
	}
	...
	t.Runtime, _ = model.ParseRuntime(runtime)
...
func (d *DB) Add(t *model.Task) error {
	...
	_, err := d.conn.Exec(`INSERT INTO tasks (id, name, status, project, branch, prompt, backend, worktree, agent_pid, session_id, pr_url, sandboxed, archived, waiting_review, runtime, remote_host, created_at, started_at, ended_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (d *DB) Update(t *model.Task) error {
	...
	res, err := d.conn.Exec(`UPDATE tasks SET name=?, status=?, project=?, branch=?, prompt=?, backend=?, worktree=?, agent_pid=?, session_id=?, pr_url=?, sandboxed=?, archived=?, waiting_review=?, runtime=?, remote_host=?, created_at=?, started_at=?, ended_at=? WHERE id=?`,
