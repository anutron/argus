...
6. **NO code path in the TUI process may write to `os.Stderr` or `os.Stdout` after `tcell.Screen.Init` runs.** The TUI's stderr/stdout IS the user's terminal. tcell does not route through them, so any direct write lands at the cursor's current position and corrupts the displayed cell state — surviving on screen until the next `screen.Sync()` because tcell's diff tracker only re-emits cells whose buffer value changed. Symptoms: torn cells, scattered log fragments, stacked status bars, mis-positioned content. **This was misdiagnosed as tcell/tmux drift for entire investigation cycles.** Guards (defense in depth): (a) `runTUI` redirects BOTH default loggers at startup — `slog.SetDefault(slog.New(slog.NewTextHandler(uxlog.Writer(), nil)))` (covers every `slog.*` call program-wide) AND `log.SetOutput(uxlog.Writer())` (covers every stdlib `log.{Print*,Fatal*,Panic*}` call); (b) `runTUI` installs an **OS-level Dup2 of fd 2 to the uxlog file** right after the slog/log redirects — this catches what (a) cannot: goroutine panic stack dumps (the Go runtime emits panic output directly to fd 2 via `runtime`, bypassing all Writer-based redirects), subprocess fd 2 inheritance, and any third-party library that writes to fd 2 directly. The original fd 2 is dup'd and restored on `runTUI` exit so post-`app.Run()` error reporting (`fmt.Fprintf(os.Stderr, "error: %v\n", err)`) still reaches the user's terminal. fd 1 is intentionally NOT redirected because `computePTYSize` needs `term.GetSize(int(os.Stdout.Fd()))` to succeed; tcell uses `/dev/tty` directly so its I/O path is independent of fd 1/2; (c) `runTUI` installs a top-level `defer recover()` that catches panics in the main goroutine and logs the goroutine stack to uxlog before re-panicking; (d) two regression tests pin the contract — `TestSlogWithUxlogWriter_DoesNotReachStderr` (Go-level Writer redirect) and `TestFd2RedirectViaDup2_CatchesRawSyscallWrites` (OS-level Dup2 redirect). Both fail the build if anyone breaks the wiring; (e) **never** call `fmt.Fprintf(os.Stderr, ...)` or `fmt.Print*(...)` from anywhere reachable from `runTUI` after `app.Run()` starts — write via `uxlog.Log` or `slog.*` instead. The daemon has its own redirects in `runDaemon`, so daemon-side calls are safe — but **any package that runs in both processes** must assume its slog/log/stderr calls might land in the TUI and must be safe under the redirects. The fix is at the program level (one line per logger plus the Dup2), NOT per call site — there are 100+ slog call sites across `internal/` (`grep -rn 'slog\.' internal/ | wc -l` ≈ 106) and that's exactly why this bug is so insidious. **Caveat:** the slog handler is constructed with `nil` `HandlerOptions`, so `slog.Debug` is silently dropped (default level is `Info`); this is intentional and matches the daemon's behavior, but if a future caller needs Debug visibility they must use `uxlog.Log` directly rather than `slog.Debug`. **Subprocesses do NOT leak**: Go's `exec.Cmd` defaults nil `Stderr`/`Stdout` to `/dev/null` (NOT inherit), the codebase has zero `cmd.Stderr = os.Stderr` callsites, and the Dup2 redirect doubly ensures any future violation lands in uxlog rather than the terminal. **No remaining known leak paths.** If a future bug report describes "stack trace appeared on screen then argus exited," check that the Dup2 in `runTUI` is still in place and the regression tests still pass — those are the load-bearing guards.
