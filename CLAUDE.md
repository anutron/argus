...
6. **NO code path in the TUI process may write to `os.Stderr` or `os.Stdout` after `tcell.Screen.Init` runs.** The TUI's stderr/stdout IS the user's terminal. tcell does not route through them, so any direct write lands at the cursor's current position and corrupts the displayed cell state — surviving on screen until the next `screen.Sync()` because tcell's diff tracker only re-emits cells whose buffer value changed. Symptoms: torn cells, scattered log fragments, stacked status bars, mis-positioned content. **This was misdiagnosed as tcell/tmux drift for entire investigation cycles.** Guards: (a) `runTUI` redirects BOTH default loggers at startup — `slog.SetDefault(slog.New(slog.NewTextHandler(uxlog.Writer(), nil)))` (covers every `slog.*` call program-wide) AND `log.SetOutput(uxlog.Writer())` (covers every stdlib `log.{Print*,Fatal*,Panic*}` call); (b) the regression test `TestSlogWithUxlogWriter_DoesNotReachStderr` in `internal/uxlog/uxlog_test.go` pins both wirings — it captures `os.Stderr` while firing slog/log calls and fails the build if any byte reaches stderr; (c) **never** call `fmt.Fprintf(os.Stderr, ...)` or `fmt.Print*(...)` from anywhere reachable from `runTUI` after `app.Run()` starts — write via `uxlog.Log` or `slog.*` instead; (d) `runTUI` installs a top-level `defer recover()` that catches panics in the main goroutine and logs the goroutine stack to uxlog before re-panicking, so a panic during app setup or `app.Run()` doesn't spray its stack dump across the terminal. The daemon has its own redirects in `runDaemon`, so daemon-side calls are safe — but **any package that runs in both processes** must assume its slog/log calls might land in the TUI and must be safe under the redirect. The fix is at the program level (one line per logger), NOT per call site — there are 100+ slog call sites across `internal/` (`grep -rn 'slog\.' internal/ | wc -l` ≈ 106) and that's exactly why this bug is so insidious. **Caveat:** the handler is constructed with `nil` `HandlerOptions`, so `slog.Debug` is silently dropped (default level is `Info`); this is intentional and matches the daemon's behavior, but if a future caller needs Debug visibility they must use `uxlog.Log` directly rather than `slog.Debug`. **Subprocesses do NOT leak**: Go's `exec.Cmd` defaults nil `Stderr`/`Stdout` to `/dev/null` (NOT inherit), and the codebase has zero `cmd.Stderr = os.Stderr` callsites. **Residual gap (known, NOT caught by guards a-d):** panics in goroutines spawned by the TUI (40+ across `internal/tui/`, `internal/agent/`, etc.) still write their stack dumps to fd 2 because the Go runtime emits panic output directly, bypassing all log packages and the main-goroutine recover. Closing this requires either wrapping every `go func()` in `defer recover()` or installing a signal handler — out of scope for the slog-leak fix, but if a future bug report describes "stack trace appeared on screen then argus exited," this is the cause.
